...
/* Source of the function with simulates population densities with the Manetsch's
 Distributed Delay Model in the Q(t) variables.
...
void simulatore2(void){
    
    // Statement for the logfile
    
    system(" echo \"simulatore2() function started \" >> logfile.log ");
    ...
    double mat[MAXRC][2]={{0}}, temp[MAXRC][2]={{0}}, initcond[MAXRC][2]={{0}}, n[MAXRC][MAXRC]={{0}}, AA[MAXRC]={0}, CC[MAXRC]={0}, errN[MAXRC][MAXRC]={{0}};
...
avanti2:
    ...
    FILE *daytemp;
        ...
        if (daytemp == 0){
            system(" echo \"Error: simulatore2() function. Failure opening file input/DailyTemperatures.txt \" >> logfile.log ");
        }
        else if (daytemp != 0){
            system(" echo \"simulatore2() function. File input/DailyTemperatures.txt succesfully opened\" >> logfile.log ");
        }
    ...
    // Ask the h value for the Manetsch's DDM model Q(t) version
...
avanti3:
    ...
    // Read the initial condition from input/InitialCondition-DDM-Q.txt
    
    FILE *monitoring;
        monitoring = fopen("./input/InitialCondition-DDM-Q.txt", "r");
        
        // Report the status of the file into the logfile
        if (monitoring == 0){
            system(" echo \"Error: simulatore2() function. Failure opening file input/InitialCondition-DDM-Q.txt \" >> logfile.log ");
        }
        else if (monitoring != 0){
            system(" echo \"simulatore2() function. File input/InitialCondition-DDM-Q.txt succesfully opened\" >> logfile.log ");
        }
    ...
    FILE *simulation1;
        ...
        // Report the status of the file into the logfile
        if (simulation1 == 0){
            system(" echo \"Error: simulatore2() function. Failure opening file input/.SimStorage/NumericalSolution.txt \" >> logfile.log ");
        }
        else if (simulation1 != 0){
            system(" echo \"simulatore2() function. File input/.SimStorage/NumericalSolution.txt succesfully opened\" >> logfile.log ");
        }
        
        // Print the content
    
        // Numerical scheme to solve the Manetsch's DDM in Q(t) variables
        ...
        for(i=0; i<=j; i++){
            ...
            for(col=1; col<=col_max+10; col++){
                ...
                n[i+1][col] = AA[i] * n[i][col-1] - CC[i] * n[i][col];
    ...
    // Report the status of the file into the logfile
        if (simulation3D == 0){
            system(" echo \"Error: simulatore2() function. Failure opening file output/simresults/simulation3D.txt \" >> logfile.log ");
        }
        else if (simulation3D != 0){
            system(" echo \"simulatore2() function. File output/simresults/simulation3D.txt succesfully opened\" >> logfile.log ");
        }
    ...
    for(h=1; h<=col_max+10; h++){
        ...
        FILE *hprint[h];
            ...
            if (hprint[col] == 0){
                char warning_1[200]={ };
                sprintf(warning_1, " echo \"Error: simulatore2() function. Failure opening file output/simresults/h=%d.txt \" >> logfile.log ", col);
                
                system(warning_1);
            }
            else if (hprint[col] != 0){
                char warning_2[200]={ };
                sprintf(warning_2, " echo \"simulatore2() function. File  output/simresults/h=%d.txt succesfully opened\" >> logfile.log ", col);
                system(warning_2);
            }
    ...
    FILE *InfoPythonGraph;
        ...
        if (InfoPythonGraph == 0){
            system(" echo \"Error: simulatore2() function. Failure opening file input/.SimStorage/SimulatedStages.txt \" >> logfile.log ");
        }
        else if (InfoPythonGraph != 0){
            system(" echo \"simulatore2() function. File input/.SimStorage/SimulatedStages.txt succesfully opened\" >> logfile.log ");
        }
