...
/* Source of the function with simulates population densities with the Generalised
 Von Foerster's integro-differential equation - Rossini PhD thesis
 https://doi.org/10.1007/s11587-020-00489-6.
...
void simulatore4(void){
    
    // Statement for the logfile
    
    system(" echo \"simulatore4() function started \" >> logfile.log ");
    ...
    int r=0, scan=0, j=0, i=0, col=0, h=0, col_max=0;
    
    int switch_lineare=0, switch_briere=0, switch_logan=0, switch_sdm=0;
    
    double mat[MAXRC][2]={{0}}, temp[MAXRC][2]={{0}}, initcond[MAXRC][2]={{0}}, boundcond[MAXRC][2]={{0}}, n[MAXRC][MAXRC]={{0}}, I[MAXRC][MAXRC]={{0}}, errN[MAXRC][MAXRC]={{0}};
...
avanti2:
    ...
    FILE *daytemp;
        ...
        if (daytemp == 0){
            system(" echo \"Error: simulatore4() function. Failure opening file input/DailyTemperatures.txt \" >> logfile.log ");
        }
        else if (daytemp != 0){
            system(" echo \"simulatore4() function. File input/DailyTemperatures.txt succesfully opened\" >> logfile.log ");
        }
...
avanti3:
    ...
    clear();
    printf("%s", "\n Do you want the standard boundary condition file? (say 'n' if you have previous monitoring) y/n \n\n");
...
avanti4:
    ...
    FILE *initprofile;
        ...
        // Report the status of the file into the logfile
        if (initprofile == 0){
            system(" echo \"Error: simulatore4() function. Failure opening file input/InitialCondition-GVFE.txt \" >> logfile.log ");
        }
        else if (initprofile != 0){
            system(" echo \"simulatore4() function. File input/InitialCondition-GVFE.txt succesfully opened\" >> logfile.log ");
        }
    ...
    FILE *monitoring;
        ...
        // Report the status of the file into the logfile
        if (monitoring == 0){
            system(" echo \"Error: simulatore4() function. Failure opening file input/BoundaryCondition-GVFE.txt \" >> logfile.log ");
        }
        else if (monitoring != 0){
            system(" echo \"simulatore4() function. File input/BoundaryCondition-GVFE.txt succesfully opened\" >> logfile.log ");
        }
   ...
   // Create the hidden file input/.SimStorage/NumericalSolution.txt containing a matrix with all the numbers calculated
    ...
    FILE *simulation1;
        ...
        // Report the status of the file into the logfile
        if (simulation1 == 0){
            system(" echo \"Error: simulatore4() function. Failure opening file input/.SimStorage/NumericalSolution.txt \" >> logfile.log ");
        }
        else if (simulation1 != 0){
            system(" echo \"simulatore4() function. File input/.SimStorage/NumericalSolution.txt succesfully opened\" >> logfile.log ");
        }
        ...
        for(i=0; i<=j; i++){
            ...
            for(col=1; col<=col_max; col++){
                ...
                m = mortality(col);
                // Rough estimation of the fertility - To replace in future!
                f = fertility(col);
                // Integer into the PDE Integro-Diff
                I[i][col] += n[i][col]*f*col;
            
                // Make the calculation
            
                n[i+1][col] =  n[i][col] - (n[i][col] - n[i][col-1])*(i/col)*temp[i][1] - n[i][col]*i*m + I[i][col];
    ...
    FILE *simulation3D;
        ...
        // Report the status of the file into the logfile
        if (simulation3D == 0){
            system(" echo \"Error: simulatore4() function. Failure opening file output/simresults/simulation3D.txt \" >> logfile.log ");
        }
        else if (simulation3D != 0){
            system(" echo \"simulatore4() function. File output/simresults/simulation3D.txt succesfully opened\" >> logfile.log ");
        }
    ...
    for(h=1; h<=col_max; h++){
        ...
        if (hprint[col] == 0){
            char warning_1[200]={ };
            sprintf(warning_1, " echo \"Error: simulatore4() function. Failure opening file output/simresults/h=%d.txt \" >> logfile.log ", col);
            
            system(warning_1);
        }
        else if (hprint[col] != 0){
            char warning_2[200]={ };
            sprintf(warning_2, " echo \"simulatore4() function. File  output/simresults/h=%d.txt succesfully opened\" >> logfile.log ", col);
            system(warning_2);
        }
    ...
    FILE *InfoPythonGraph;
        ...
        if (InfoPythonGraph == 0){
            system(" echo \"Error: simulatore4() function. Failure opening file input/.SimStorage/SimulatedStages.txt \" >> logfile.log ");
        }
        else if (InfoPythonGraph != 0){
            system(" echo \"simulatore4() function. File input/.SimStorage/SimulatedStages.txt succesfully opened\" >> logfile.log ");
        }
