...
Notice that the hidden directories will be automatically hidden with the 'make all' command within Makefile, when you compile and build the executable of EntoSim. On this git repository they are not hidden.
******************************************************************************
...
The first tab contain a 3D-plot with the population density over time and through the life stages, the second tab reports a plot with an overlap of all the numerical solutions and the third tab contains multiple plots, one for each life stage simulated.  
